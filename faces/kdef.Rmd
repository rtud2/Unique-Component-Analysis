...
libraries <- c("data.table", "MASS", "ggplot2", "splines","gridExtra","uca", "imager", "future.apply", "RSpectra", "microbenchmark")
...
happy <- data.table(happy)
neutral <- data.table(neutral)
angry <- data.table(angry)
A = t_happy/sqrt(nrow(t_happy)-1)
B = list(t_neutral/sqrt(nrow(t_neutral)-1), t_angry/sqrt(nrow(t_angry)-1))

vals = c(2,2)

microbenchmark(bisection2(A, B[[1]]), bisection(cov_happy, cov_neutral), times = 1)
tmp <- bisection(cov_happy, cov_neutral)
tmp2 <- bisection2(A, B[[1]])

tmp1_1_time2 %<-% system.time(tmp1_1 <- uca(cov_happy, list(cov_neutral, cov_angry), nv = 2, method = "cov"))

system.time(tmp1_2 <- uca(t_happy, list(t_neutral, t_angry), nv = 2, method = "data"))

microbenchmark(data = uca(t_happy, list(t_neutral, t_angry), method = "data"),
               cov = uca(cov_happy, list(cov_neutral, cov_angry)), times = 1)

microbenchmark(happy_double_neutral = uca(t_happy, list(t_neutral), method = "data"), uca(cov_happy, cov_neutral, method = "cov"),times = 2)


left <- do.call(cbind, lapply(append(list(A), B), t))
right <- do.call(rbind,append(list(A),B))

all.equal(broken_svd_cpp(left, right , 2), broken_svd_R(left, right, 2))
microbenchmark(t(t(left) * mult_v), sweep(left, 2, mult_v, "*"), cbind(old_left, 2*t(B2)))

mult_v = c(2, 2)
B = list(t_neutral, t_angry)
microbenchmark(do.call(cbind, Map("*", -mult_v, lapply(B, t))),
t(do.call(rbind, Map("*", -mult_v, B))))
microbenchmark(lapply(B, t))
```
...
toEigenfaces <- function(dat){
  eigenfaces <- data.table(do.call(rbind,
                        future_lapply(1:ncol(dat), function(zz){
