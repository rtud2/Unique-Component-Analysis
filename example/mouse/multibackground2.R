...
bg2 <- data.matrix(mouse_dt[Treatment == "Saline" & Behavior == "S/C" & Genotype ==cs_gentype, lapply(.SD, scale), .SDcols = -c("MouseID","Genotype","Treatment","Behavior","class")])
# bg3 <- data.matrix(mouse_dt[Treatment == "Memantine" & Behavior == "C/S" & Genotype == cs_gentype, lapply(.SD, scale), .SDcols = -c("MouseID","Genotype","Treatment","Behavior","class")])

bg12 <- data.matrix(mouse_dt[Behavior == "S/C" & Genotype == cs_gentype, lapply(.SD, scale),.SDcols = -c("MouseID","Genotype","Treatment","Behavior","class")])
...
cpca_stack_0p5 <- eigs_sym(target_cov - 0.5*bg12_stack_cov, which = "LA", k = 2)$vectors
cpca_stack_1 <- eigs_sym(target_cov - 1*bg12_stack_cov, which = "LA", k = 2)$vectors
cpca_stack_5 <- eigs_sym(target_cov - 5*bg12_stack_cov, which = "LA", k = 2)$vectors
cpca_stack_10 <- eigs_sym(target_cov - 10*bg12_stack_cov, which = "LA", k = 2)$vectors
...
cpcapp_bg12 <- eigen(solve(bg12_stack_cov) %*% target_cov, 2)$vector[,1:2]
...
#  data.table(target_data %*% cpcapp_bg3, "cPCA++", paste0("Saline-S/C-",cs_gentype), target_data_labs),
  data.table(target_data %*% cpcapp_bg12, "cPCA++", paste0("Stack All (",cs_gentype,")"), target_data_labs)
  )
...
ggsave(paste0("Mouse_stack_cpc",cs_gentype,".png"), other_res, width = 11, height = 8, units = "in")
...
# uca_bg3 <- uca(A = target_cov, B = bg3_cov, method = "cov")$vectors
uca_stack <- uca(A = target_cov, B = bg12_stack_cov, method = "cov")$vectors
uca_split <- uca(A = target_cov, B = list(bg1_cov, bg2_cov), method = "cov")$vectors
cpca_pp <- eigen(solve(bg12_stack_cov) %*% target_cov)$vectors[,1:2]
...
plot_memantine_saline <- rbind(
  ...
  data.table(target_data %*% uca_bg2, paste0("UCA (Saline-S/C-",cs_gentype,")"), target_data_labs),
...
ggsave(paste0("Mouse_split_stack_no_CS_",cs_gentype,".png"), mouse_split_stack, width = 11, height = 8, units = "in")
