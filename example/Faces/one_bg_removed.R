...
for (emo in seq_along(kdef_emotions)) {
    ...
    saveRDS(res_list[[emo]], paste0("example/Faces/One_Background_Removed/", kdef_emotions[emo], "removed.rds"))
}
...
#Plotting the images
 for (emo in seq_along(res_list)) {
    ...
    geom_point(aes(x = V1, y = V2, color = Emotion, shape = Emotion)) +
    ...
    scale_shape_manual(values = c(2,2,2,1,1,2,1)) +
