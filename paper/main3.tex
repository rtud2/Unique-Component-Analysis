...
%     \centering
%     \includegraphics[height = 8.5cm,width=\textwidth]{figure/Afraid_projected_1_2_removed.png}
%     \caption{}
%     \label{fig:faces-projected}
% %Faces projected onto the first two components found by PCA, UCA with a pooled background, and UCA with backgrounds split
%     \end{subfigure}%
...
%     \centering
%     \includegraphics[height=8.5cm,width=\textwidth]{figure/all_Afraid_removed.png}
%     \caption{}
%     \label{fig:faces-eigenfaces}
%     %Top 5 Eigenfaces of Female Emotions by found by PCA, UCA with a pooled background, and UCA with backgrounds split
%     %\caption{Top five eigenfaces of Female Actors from KDEF \cite{Calvo2008}}
...
% }
% \caption{
%   Results of analysis of KDEF faces.  (a) Each emotion averaged over all female actors. (b) Faces projected onto the first two components found by PCA, UCA with a pooled background, and UCA using multiple backgrounds. (c) Top 5 eigenfaces of female emotions produced by different analyses.}
% \end{figure}
...
% UCA only finds the optimal contrastive parameter using Lagrange multipliers and does not remedy a poor background choice.
...
\end{align}
We note that strong duality holds because our primal problem is quadratic and the constraints satisfy Slater's condition\cite{boyd2004convex}: covariance matrix $B$ is positive semi-definite, thus for all $v$, $v^T B v \geq 0$, implying existance of $v$ such that $v^T B v \leq 1$. 
...
We use an iterative algorithm (L-BFGS-B) \cite{byrd1995limited} to search for Lagrange multiplier $\lambda$ such that the associated eigenvectors $v_\lambda$ satisfies equation \ref{eq:3}. The resulting $v_\lambda$ also solves the primal problem due to strong duality.
