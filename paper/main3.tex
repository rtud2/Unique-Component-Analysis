...
For example, in Section \ref{sec:mouse}, we analyze proteomics data from of normal and trisomic mice that have undergone shock therapy and/or drug therapy. One goal is to identify patterns uniquely associated with normal mice. However, the dominant modes of variation in the dataset arise from the shock treatment and the trisomic gene, which are not of interest. Therefore, we contrast the data with a background dataset of proteomics data from shock-treated and drug-treated trisomic mice. As a result, the remaining patterns of variation unique to the target dataset reveal features specific to normal mice.
...
\caption{\textbf{The faces in (b) look too squashed. Can you come up with a different arrangement of the subfigures? Also since some of the colors in the PCA are too similar, I had another idea, do you think we can do shapes as well? So like the ``bad'' emotions (anger, digust, etc.) get different shapes?}
...
The tuning parameter $\lambda$ measures how much to penalize the background data covariance. When $\lambda = 0$, background variation isn't accounted for, thus the cPCA is reduced to PCA. As $\lambda$ increases, the relative background variation becomes more dominant, causing $v_\lambda$ \textbf{What's $v_\lambda$? Need to define it} to focus on directions which minimize background variation rather than maximizing the target. For very large values of $\lambda$, $v_\lambda$ is equivalent to PCA after projecting the target data onto the nullspace of the background data.  The authors of cPCA suggested that $\lambda$ be chosen using spectral clustering via a parameter sweep of logarithmically spaced candidate values.
...
\textbf{Why does cPCA++ enforce $v^\top B v = 1$? We should explain.}
...
However, constraining $v^T Bv = 1$ while leaving $v^T v$ unconstrained results in eigenvectors that are not norm 1 and explain near zero variability in the target. \textbf{How is this possible? Need to explain. Basically we need to explain how cPCA++ works before we explain UCA.}
...
We have released a R implementation of \href{https://urldefense.com/v3/__https://github.com/rtud2/Residual-Dimension-Reduction/tree/master/uca*7D*7BUCA__;JSU!!DZ3fjg!sGIYM1o_8SSJW1QbOZ62mPidAZTlpAuO5P4v-6IHek6bOPrBOHX6LTxlOlrh0Zk90pI$  on GitHub}. We have implemented both Product SVD on the data matrix and eigendecomposition on the contrastive covariance matrix and allow the background to take either a single background or a list of backgrounds. The GitHub repository also includes R markdown and datasets that reproduce most of the figures in this paper and in the Supplementary.

\subsection{Data availability}
Datasets that have been used to evaluate UCA in this paper are publicly available from the authors of the original studies. The mouse proteomics data are available from the \href{https://urldefense.com/v3/__https://archive.ics.uci.edu/ml/datasets/Mice*Protein*Expression*7D*7BUCI__;KyslJQ!!DZ3fjg!sGIYM1o_8SSJW1QbOZ62mPidAZTlpAuO5P4v-6IHek6bOPrBOHX6LTxlOlrhjXCSUAo$  Machine Learning Repository} \cite{Higuera} and the Karolinska Directed Emotional Faces (KDEF) are available from the \href{https://urldefense.com/v3/__https://kdef.se/*7D*7BKDEF__;JSU!!DZ3fjg!sGIYM1o_8SSJW1QbOZ62mPidAZTlpAuO5P4v-6IHek6bOPrBOHX6LTxlOlrhF2-OqmU$ } website  \cite{Calvo2008}.
