...
The diagonal elements of $D^2$ be described as the variance explained by each dimension. When $X$ is centered:

\begin{align*}
\text{Cov}\left(XV\right) &= V^T\text{Cov}\left(X\right)V\\
&=V^TX^TXV\\
&=V^TVD^2V^TV\\
&=D^2
\end{align*}
...
cPCA = function(target, bg, n_components = 2, alpha = 1, standardize = T, return_all = F){
  ...
  v_top <- eigen(sigma)$vectors[,1:n_components]
  #reduced_target <- og_target %*% v_top
  reduced_target <- target %*% v_top
  reduced_bg <- bg %*% v_top
...
Advantages:
  ...
  * Avoid calculating the eigenvalue decomposition of the difference in the variance-covariance matrices. Just calculating the difference between the variance-covariance matrices when $p$ is large is a big memory problem.
...
3. Project the target onto the orthogonal complement of the background, $X_{target} \left[I - P_{bg}\right]$
    + $X_{target} \left[I - P_{bg}\right]$ projects $X_{target}$ onto the rowspace of $\left[I - P_{bg}\right]$
    + Since $\left[I - P_{bg}\right]$ is a projection matrix, $\left[I - P_{bg}\right] = \left[I - P_{bg}\right]^T$ means that $X_{target}$ is also projected onto the column space of $\left[I - P_{bg}\right]$ 
    + In the event that $X$ is high dimensional, it's easier to distribute the $X_{target}$ variable, and Calcualte $X_{bg} - \left(X_{bg}v_{bg} \right)v_{bg}^T$ 
...
* Function inputs:
  * `target`: Target dataset - dataset of interest
  * `bg`: Background dataset
  * `n_components`: number of Principal components to calculate for the target data, after being projected onto the orthogonal complement of the background.  if NULL, then n_components chooses automatically based on finding the best linear spline with respect to squared-error.
  * `bg_components`: number of background principal components used. Tuning parameter because this affects the span of the Orthogonal Complement.  if NULL, then bg_components chooses automatically based on finding the best linear spline with respect to squared-error.
...
* Function Inputs:
  * `d` the vector of diagonals from an SVD
  * `total_component` default is 20. The most number of components to include when fitting linear single knot splines 
  * `return_all` default FALSE.  If TRUE, then data.frame of mean squared error for the linear spline with knot for each principal component is outputted

* Function Output:
either return the number optimal number of principal components or additionally, the data.frame of squared error for the linear spline with knot for each principal component is outputted
...
Dave's version of code OG:

  1. did not project. $Xv$ was calculated, but not $X(I -P_{bg}) \Leftrightarrow Xv_{k+1:p}v_{k+1:p}^T$
  2. Projecting using the last columns vs calculating the $I -P_{bg}$ yielded the same result

```{R, eval = F}
X = targ[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")]
B = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")]

v = svd(scale(B, center = TRUE, scale = apply(B, 2, sd)), nv = ncol(B))$v
k = 2 ## first k PCs of B explain variation in background
newX = scale(X, center = colMeans(B), scale = apply(B, 2, sd)) %*% v[,-(1:k)] ## part of data not explained by v
newX = scale(newX, center = TRUE, scale = apply(newX, 2, sd))

reduced_target = data.table(scale(newX, center = TRUE, scale = apply(newX, 2, sd)) %*% svd(newX)$v[,1:2])

```


Stepping through the scale function, `scale(X)` is equivalent to `scale(X, center = T, scale = apply(X, 2, sd))`. For example:
...
B = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")]
expect_equivalent(scale(B), scale(B, center = T, scale = apply(B, 2, sd)))
```

Cleaned up for readability

```{R}
dave_occPCA = function(target,
                       bg,
                       n_components = 2,
  ...
  v = svd(scale(B))$v # SVD on scaled background data, no difference here
    ...
    newX = scale(X, center = colMeans(B), scale = apply(B, 2, sd)) %*% v[,-(1:bg_components)] ## part of data not explained by v
    newX = scale(newX, center = TRUE, scale = apply(newX, 2, sd))
    reduced_target = data.table(scale(newX, center = TRUE, scale = apply(newX, 2, sd)) %*% svd(newX)$v[,1:n_components])

  return(reduced_target)  
}

mod_dave_occPCA = function(target,
                       bg,
                       n_components = 2,
                       bg_components = 2 ## first k PCs of B explain variation in background
                       ){
  ...
  newX = scale(X, center = colMeans(B), scale = apply(B, 2, sd)) %*%  tcrossprod(selected_v) # why scale to the background??
  #newX = scale(newX)
  reduced_target = data.table(X %*% svd(scale(newX))$v[,1:n_components])
  return(reduced_target)  
}
...
**Why scale the target data to the background**
**You're visualizing the data via principal components. Should visualized data be scaled**
...
#### rPCA
```{R,  fig.align='center'}
results_occPCA <- rPCA(target = targ[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg_components = NULL,
     n_components = 2,
     return_all = T)
...
occ_reduced_target <- cbind(occ_reduced_target,  targ[, .SD, .SDcols = c("Genotype")], "rPCA")
...
tuning_bg <- lapply(search_grid, function(xx) data.table(rPCA(target = targ[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg_components = xx,
     n_components = 2,
     return_all = F)))

tuning_bg_plot <- data.table(rbindlist(tuning_bg),
                                sort(rep(search_grid, nrow(targ))),
                                unlist(rep(targ[, "Genotype"],length(search_grid))))
...
## Checking Dave's Code

tuning_g for dave's code

### Using Dave's pretty much OG code

```{R}
search_grid <- 1:10

#Dave's code in function form
tuning_bg_dave1 <- lapply(search_grid, function(xx) dave_occPCA(target = targ[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg_components = xx))

tuning_bg_dave_plot1 <- data.table(do.call(rbind, tuning_bg_dave1),
                                sort(rep(search_grid, nrow(targ))),
                                unlist(rep(targ[, "Genotype"],length(search_grid))))

setnames(tuning_bg_dave_plot1, c("PCA.1", "PCA.2", "alpha", "Down.Syndrome"))

ggplot(data = tuning_bg_dave_plot1)+
  geom_point(aes(x = PCA.1, y = PCA.2, color = Down.Syndrome), alpha = 0.6)+
  facet_wrap(~alpha, scales = "free", nrow = 2)+
  theme(legend.position = "bottom")+
  labs(title = "Mouse Down Syndrome Data: occPCA with different Background Components [Daves Code]")

```

### Modified to remove a bunch of scaling to verify same results

```{R}
#Robin modifies Dave's code in function form
tuning_bg_dave2 <- lapply(search_grid, function(xx) mod_dave_occPCA(target = targ[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg = background[, .SD, .SDcols = -c("Genotype", "Treatment", "Behavior")],
     bg_components = xx))

tuning_bg_dave_plot2 <- data.table(do.call(rbind, tuning_bg_dave2),
                                sort(rep(search_grid, nrow(targ))),
                                unlist(rep(targ[, "Genotype"],length(search_grid))))

setnames(tuning_bg_dave_plot2, c("PCA.1", "PCA.2", "alpha", "Down.Syndrome"))

ggplot(data = tuning_bg_dave_plot2)+
  geom_point(aes(x = PCA.1, y = PCA.2, color = Down.Syndrome), alpha = 0.6)+
  facet_wrap(~alpha, scales = "free", nrow = 2)+
  theme(legend.position = "bottom")+
  labs(title = "Mouse Down Syndrome Data: occPCA with different Background Components [Robin mod Daves Code]")
